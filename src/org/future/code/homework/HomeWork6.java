 ...
 */
    ...
    public static abstract class Instrument implements Playable {
        ...
        public Instrument(int times, String sound) {
            this.sound = sound;
            this.times = times;
        }
        ...
        public List<String> play() {
            ...
            str.add((getSound() + " ").repeat(getTimes()));
            return str;
        }

        List<String> playWith1 = new ArrayList<>();

        public List<String> playWith(Playable playable) {
            playWith1.addAll(this.play());
            playWith1.addAll(playable.play());
            return playWith1;
        }
    }
    ...
    public static class Orchestra implements Playable {
        ...
        public Orchestra(Instrument... instruments) {
            for (int i = 0; i < instruments.length; i++) {
                getInstruments().add(instruments[i]);
            }
        ...
        public List<String> play() {
            for (int i = 0; i < instruments.size(); i++) {
                play1.addAll(instruments.get(i).play());
            }
            return play1;
        }
        ...
        public List<String> playWith(Playable playable) {
            for (int i = 0; i < instruments.size(); i++) {
                playWith1.addAll(instruments.get(i).play());
            }
            playWith1.addAll(playable.play());
            return playWith1;
        }
